...
static double time_tamm_queue_active(int64_t N, int n_tasks, int active_ranks, tamm::ProcGroup world_pg) {
    ...
    std::cout << "rank " << my << " tamm_enter N " << N << " n_tasks " << n_tasks
              << " active_ranks " << active_ranks << " world_size " << sz << std::endl;
    ...
    if (my < active_ranks) {
        ...
        while (true) {
            ...
            std::cout << "rank " << my << " tamm_task_begin idx " << idx
                      << " M " << M << " bt " << bt << std::endl;
            ...
            std::cout << "rank " << my << " tamm_schedule_contr_post idx " << idx << std::endl;

            sch.deallocate(A, B, C);
            std::cout << "rank " << my << " tamm_schedule_dealloc_post idx " << idx << std::endl;
        }
    } else {
        std::cout << "rank " << my << " tamm_nonworker_idle" << std::endl;
    }

    // Everyone executes once to avoid mismatched collectives inside TAMM
    std::cout << "rank " << my << " tamm_execute_pre (all ranks)" << std::endl;
    sch.execute(ec.exhw(), false);
    std::cout << "rank " << my << " tamm_execute_post (all ranks)" << std::endl;
    ...
    double dt = (my == 0 ? (t1 - t0) : 0.0);
    if (my == 0) std::cout << "rank 0 tamm_time " << std::fixed << std::setprecision(6) << dt << " s" << std::endl;
...
static double time_slate_svd_queue_active(int64_t N, int n_tasks, int active_ranks, tamm::ProcGroup world_pg) {
    ...
    int my = world_pg.rank().value();
    int sz = world_pg.size().value();
    std::cout << "rank " << my << " slate_enter N " << N << " n_tasks " << n_tasks
              << " active_ranks " << active_ranks << " world_size " << sz << std::endl;
    ...
    // Everyone executes once to keep collectives matched if SLATE/TAMM do any
    std::cout << "rank " << my << " slate_execute_pre (all ranks)" << std::endl;
    // SLATE ops are sync by call; nothing to execute here, but keep a print for symmetry.
    std::cout << "rank " << my << " slate_execute_post (all ranks)" << std::endl;
    ...
    double dt = (my == 0 ? (t1 - t0) : 0.0);
    if (my == 0) std::cout << "rank 0 slate_time " << std::fixed << std::setprecision(6) << dt << " s" << std::endl;
