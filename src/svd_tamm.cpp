...
static double time_tamm_contractions(int64_t N, int n_contr, tamm::ProcGroup world_pg) {
    ...
    // one-rank subgroup per process (one GPU per rank)
    ...
    // global queue across all ranks
    tamm::AtomicCounterGA ac{world_pg, 1};
    ac.allocate(0);

    // rank-0 timing between barriers to capture critical path
    world_pg.barrier();
...
static double time_slate_svds(int64_t N, int n_svd, tamm::ProcGroup world_pg) {
    ...
    // use the per-rank self communicator: TAMM provides the MPI_Comm via pg.comm()
    // since we are not including mpi.h here, we use the single-rank subgroup’s comm
    // by constructing a subgroup of size 1 again
    tamm::ProcGroup self_pg = tamm::ProcGroup::create_subgroups(world_pg, 1);
    ...
    while (true) {
        ...
        // slate matrix owned by this rank only
...
int main(int argc, char** argv) {
    // TAMM initializes the runtime (including MPI/GA) internally.
    tamm::initialize(argc, argv);

    tamm::ProcGroup world_pg = tamm::ProcGroup::create_world_coll();
    const int rank = world_pg.rank().value();
    const int size = world_pg.size().value();

    // defaults; override by CLI: n_contr n_svd minN maxN step csv
    int n_contr = 100;
    int n_svd   = 100;
    int64_t minN = 128, maxN = 512, step = 128;
    ...
    for (int64_t N = minN; N <= maxN; N += step) {
        if (rank == 0) std::cout << "N " << N << " contractions start\n";
        double t_contr = time_tamm_contractions(N, n_contr, world_pg);
        if (rank == 0) std::cout << "N " << N << " contractions done " << std::fixed << std::setprecision(6) << t_contr << " s\n";

        if (rank == 0) std::cout << "N " << N << " svd start\n";
        double t_svd = time_slate_svds(N, n_svd, world_pg);
        if (rank == 0) std::cout << "N " << N << " svd done " << std::fixed << std::setprecision(6) << t_svd << " s\n";
        ...
        if (rank == 0) {
            ...
            ofs.close();
            std::cout << "N " << N << " total " << std::fixed << std::setprecision(6) << t_total << " s\n";
        }
    }
