...
template<typename T>
static void two_site_update(i64 D, i64 Dmax, i64 tilesz, const std::string& gate_kind, unsigned long long seed, tamm::ProcGroup self_pg) {
...
int main(int argc, char** argv) {
    ...
    if(r == 0) {
        ...
        std::cout << "config bond_dim " << args.bond_dim << " max_bond_dim " << args.max_bond_dim << " gates " << args.gates << " gate " << args.gate << " seed " << args.seed << " tilesz " << args.tilesz << std::endl;
    }
    world_pg.barrier();
    ...
    while(true) {
        ...
        two_site_update<double>(args.bond_dim, args.max_bond_dim, args.tilesz, args.gate, args.seed + static_cast<unsigned long long>(idx), self_pg);
    }
    world_pg.barrier();
